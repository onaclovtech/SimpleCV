...
class NeuralNetworkClassifier:
    """
    This class encapsulates a Neural Network Classifier.
    See:
    ...
    def train(self,images,classNames,disp=None,subset=-1,savedata=None,verbose=True):
        ...
        self.mClassifier = Orange.classification.neural.NeuralNetworkLearner(self.mDataSetOrange)
        correct = 0
        incorrect = 0
        ...
        if( len(self.mClassNames) > 2 ):
            crossValidator = orngTest.learnAndTestOnLearnData([orange.NeuralNetworkLearner],self.mDataSetOrange)
    ...
    def test(self,images,classNames,disp=None,subset=-1,savedata=None,verbose=True):
        ...
        confusion = 0
        if( len(self.mClassNames) > 2 ):
            crossValidator = orngTest.learnAndTestOnTestData([orange.NeuralNetworkLearner()],self.mDataSetOrange,testData)
            confusion = orngStat.confusionMatrices(crossValidator)[0]
